...
void runSnake() {
  ...
  // NEU: Langsamerer Start (250) und nicht mehr so extrem schnell werdend (Max 100 statt 60)
  long speed = max(100L, 250L - (long)scoreSnake * 4); 
...
// ==========================================
// TETRIS (Verbessert mit Hard-Drop auf Taste A)
// ==========================================
#define TET_COLS 10
#define TET_ROWS 20
...
void runTetris() {
  ...
  // NEU: Hard-Drop Funktion (BTN_A)
  if (BTN(BTN_A) && !btnA_pressed) {
    btnA_pressed = true;
    tetDrawPiece(true);
    while (!tetCollides(tetPX, tetPY + 1, tetPiece, tetRot)) {
      tetPY++;
    }
    tetLock();
    tetClearLines();
